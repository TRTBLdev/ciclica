...
import { CheckSquare, Square, RotateCw, X, Lock, Edit2, Save, ChevronDown, ChevronUp, Plus, Repeat, CheckCircle2, ArrowUpFromLine, Folder, Play, ArrowUpRight, Search, ChevronRight, ArrowUp, ArrowDown, Layers } from 'lucide-react';
...
import { HabitDurationSummary, RoutineDurationSummary } from '../domain/habitDuration';
...
export function getTypeIcon(type: TaskType, className?: string) {
  ...
  switch (type) {
    case 'Proyecto':
      return <Layers className={getClassName("w-3.5 h-3.5 stroke-[2] shrink-0")} />;
    case 'Rutina':
      return <Repeat className={getClassName("w-3.5 h-3.5 stroke-[2] shrink-0")} />;
    case 'Hábito':
      return <CheckCircle2 className={getClassName("w-3.5 h-3.5 stroke-[2] shrink-0 text-emerald-600 dark:text-emerald-500")} />;
    case 'Tarea':
      return <CheckCircle2 className={getClassName("w-3.5 h-3.5 stroke-[2] shrink-0")} />;
    case 'Pulso':
      return (
...
export default function TaskItem({
  ...
  const recurringDuration = task.type === 'Hábito' && durationSummary
    ? {
      actualHours: durationSummary.currentActualHours,
      estimatedHours: durationSummary.estimatedHours,
    }
    : task.type === 'Rutina' && routineDurationSummary
      ? {
        actualHours: routineDurationSummary.currentActualHours,
        estimatedHours: routineDurationSummary.estimatedHours,
      }
      : null;
  const trackedHours = recurringDuration
    ? recurringDuration.actualHours
    : getTrackedDuration();

  const plannedHours = recurringDuration
    ? recurringDuration.estimatedHours
    : task.type === 'Proyecto'
      ? allTasks
  ...
  const recurringDurationMetadata = recurringDuration
    && (recurringDuration.actualHours > 0 || recurringDuration.estimatedHours > 0)
    ? (
      <DurationComparison
        key="duration-comparison"
        actualHours={recurringDuration.actualHours}
        estimatedHours={recurringDuration.estimatedHours}
  ...
  const contextualMetadata: Array<React.ReactNode | null> = context === 'today' ? [
    ...
    recurringDuration ? null : trackedHours > 0 ? `${trackedHours.toFixed(2)}h` : null,
  ] : [
    nextAppearanceDate && !isProjectChild ? <CompactDate label="Próx." value={nextAppearanceDate} /> : null,
    recurringDuration
      ? null
  ...
  const compactMetadata = recurringDurationMetadata
    ? [
      recurringDurationMetadata,
      ...limitCardMetadata<React.ReactNode>([...temporalMetadata, ...contextualMetadata], 2),
    ]
      ...
      <div className="flex items-start gap-3 md:gap-4 w-full">
        ...
        <div className="flex flex-col items-center gap-1.5 shrink-0 w-6 pt-1">
          ...
          {(!!task.notes
            ...
            || hasSubtasks
            || (task.type === 'Hábito' && (durationSummary?.completedCycleCount || 0) > 0)) && (
              <button
        ...
        <section className="mt-3 flex w-full flex-col gap-3 pl-4 pr-2 md:pl-6" aria-label={`Detalles de ${task.text}`}>
          {task.type !== 'Pulso' && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-mono text-text-dim">
              ...
              ) : task.type === 'Hábito' && durationSummary ? (
                <ActivityDurationAverage
                  lastActivity={itemTemporalIndicators.find(indicator => indicator.kind === 'activity')?.title}
                  averageHours={durationSummary.completedCycleAverageHours}
                  count={durationSummary.completedCycleCount}
                  className="m-0"
                />
              ) : task.type === 'Rutina' && routineDurationSummary ? (
                <ActivityDurationAverage
